...
from config import EQUITY_FILE, AUDIT_FILE, DASHBOARD_PATH, CONFIG
    ...
    try:
        ...
        for tid, pos in state["positions"].items():
            ...
            pos_data[tid] = {
                ...
                "pnl_fmt": f"${pnl:+.2f}",
                "pct_fmt": f"{pct:+.1f}%",
                "start": time.strftime('%H:%M', time.localtime(start_ts)) if start_ts else "-",
                "end": time.strftime('%m/%d %H:%M', time.localtime(end_ts)) if end_ts else "-",
