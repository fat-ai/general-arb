...
# Optional hard upper bound on trade timestamps pulled into the sort. A rebuild
# to day N never consumes trades past its stop boundary, but the ORDER BY still
# sorts (and spills to temp disk) every row the query returns. Setting this to
# the target boundary ts + ~1 day keeps the external sort to just the rows the
# run will actually consume. Rows <= the cutoff sorted by (ts, id) are exactly
# the prefix of the full sorted stream, so state at the stop day is unchanged.
# Unset = +inf = no bound (normal runs unchanged).
QUERY_TS_MAX = float(os.environ.get("SIM_QUERY_TS_MAX", "inf"))
_TS_MAX_CLAUSE = f"AND ts <= {QUERY_TS_MAX}" if math.isfinite(QUERY_TS_MAX) else ""
...
def main():
    ...
    try:
        ...
        con.execute(f"SET max_temp_directory_size = '{os.environ.get('SIM_DUCKDB_TMP_MAX', '200GB')}';")
