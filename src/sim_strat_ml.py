...
def extract_features(risk_vol, price, trade_ts, end_ts, question, is_long):
    """Converts raw trade data into a 12-feature array for XGBoost."""
    ...
    is_crypto = 1 if re.search(r"bitcoin|btc|eth|ethereum|solana|crypto|binance", q_lower) else 0
    is_politics = 1 if re.search(r"election|biden|trump|senate|gop|democrat|harris", q_lower) else 0
    is_sports = 1 if re.search(r"nfl|nba|ufc|premier league|super bowl|olympics", q_lower) else 0
    is_pop_culture = 1 if re.search(r"oscars|box office|taylor swift|mrbeast|tweet", q_lower) else 0
    
    word_count = len(q_lower.split())
    has_date = 1 if re.search(r"2024|2025|2026|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec", q_lower) else 0
    
    is_long_int = 1 if is_long else 0
    
    return [log_vol, price, days_to_res, hour, day_of_week, is_crypto, is_politics, is_sports, is_pop_culture, word_count, has_date, is_long_int]
