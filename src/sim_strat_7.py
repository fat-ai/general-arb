...
def _resolve_positions_core(user_ids, is_yes, packed_data, p_trues, stakes,
    ...
    Confirmed outcomes only — void/unknown resolutions (skip_history) release
    exposure and touch nothing else."""
    yes_idx = 0
    no_idx = 0
...
def resolve_market(r_cid: str, outcome: float, outcome_label: str, current_sim_day, state: BayesianState, day_yes_updates, day_no_updates, outcome_confirmed: bool = True):
    ...
    # NOTE: this label test must stay in lockstep with bet_on_is_yes in the row
    # pre-pass (~line 1408) — the pair defines the effective-YES frame convention;
    # flipping one without the other corrupts every token-1 history bit.
    ...
    if r_cid in state.first_bets_pending:
        ...
        if not skip_history:
            ...
            # because token_outcome_label is synthetic ("yes" = token 0,
            # download_data_sql.py:202); this removes that hidden dependency.
            token_won = outcome > 0.5
...
def main():
    ...
    if CONFIG.get('exclude_hostile_markets', True):
        ...
        # hostile markets' TOKEN ids here and anti-join them in the DuckDB trade query.
        # Three-way experiment: baseline / bets-only (flag off) / bets+scoring (flag on).
        if CONFIG.get('exclude_hostile_from_scoring', False) and _exprs:
            _hc = pl.read_parquet(MARKETS_PATH, columns=_cols + ['contract_id'])
            HOSTILE_CIDS_TBL = (_hc.filter(_cond)
                                .select(pl.col('contract_id').cast(pl.Utf8).str.strip_chars().alias('cid'))
                                .drop_nulls().unique())
            log.info(f"Hostile SCORING filter ON: {HOSTILE_CIDS_TBL.height:,} tokens "
                     f"({len(HOSTILE_MARKETS):,} markets) excluded from user-score ingestion.")
        elif CONFIG.get('exclude_hostile_from_scoring', False):
            log.warning("exclude_hostile_from_scoring requested but no rule fields present -- ignored.")
    else:
    ...
    for market in markets_pl.iter_rows(named=True):
        ...
        # Slot the CID into the parent market tracker. Only the PAIRING matters —
        # these slots feed the sibling_cid cross-link below and nothing else — so
        # pair by fill order per market_id instead of trusting token_outcome_label.
        # The label is synthetic today ("yes" = token 0, download_data_sql.py:202),
        # making fill-order identical on current data, and this keeps working if
        # the label ever carries real outcome names (e.g. team markets).
        if result_map[mid]['yes_cid'] is None:
            result_map[mid]['yes_cid'] = cid
        elif cid != result_map[mid]['yes_cid']:
    ...
    try:
        ...
        # SIGNAL-LEVEL hostile filter: anti-join the ingestion stream against the
        # hostile token set (registered from the SAME python-built rule as the bet
        # gate -- single source of truth, no SQL re-implementation to drift).
        hostile_scoring_clause = ""
        if HOSTILE_CIDS_TBL is not None and HOSTILE_CIDS_TBL.height > 0:
            con.register("hostile_cids", HOSTILE_CIDS_TBL.to_arrow())
            hostile_scoring_clause = (
        ...
        # Markets whose end passed without a confirmed outcome in the parquet —
        # routed through the exposure-only void path (no brier/history/calibration).
        ...
        if unknown_resolved_count:
            log.info(f"⚠️ {unknown_resolved_count:,} markets ended without a confirmed outcome "
                     f"(exposure released only — no brier/history/calibration impact).")
