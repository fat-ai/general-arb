...
def fill_gaps():
    ...
    existing_columns = pq.read_schema(market_file).names
    ...
    file_path = 'added_ids.txt'

    try:
        # 1. Open the file in read mode ('r')
        with open(file_path, 'r') as file:
            # 2. Use list comprehension to clean and convert each line
            # .strip() removes newlines (\n) and spaces
            # int() converts the string to a number
            id_list = [int(line.strip()) for line in file if line.strip()]
    
        # Display the result
        print("Successfully loaded IDs:")
        print(id_list)
...
                            f.write(f"{mid}\n")
                        print(f"   [{i+1}/{len(id_list)}] Skipped Market {mid} (Zero Vol)", end='\r')
                        success = True
                        break
